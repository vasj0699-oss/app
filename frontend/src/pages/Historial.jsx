        ...
        <AppLayout title="Historial" subtitle="Consulta de bitácoras registradas">
            <div className="bg-white rounded-[10px] border border-neutral-200 p-4 mb-5 shadow-[0_2px_8px_-2px_rgba(28,28,26,0.08)]">
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                    ...
                    <div><label className="text-xs">Desde</label><input type="date" value={filters.fecha_inicio} onChange={(e) => setFilters({ ...filters, fecha_inicio: e.target.value })} className="ip" /></div>
                    <div><label className="text-xs">Hasta</label><input type="date" value={filters.fecha_fin} onChange={(e) => setFilters({ ...filters, fecha_fin: e.target.value })} className="ip" /></div>
                </div>
                        ...
                        <div key={b.id} className="bg-white rounded-[10px] border border-neutral-200 shadow-[0_2px_8px_-2px_rgba(28,28,26,0.08)]">
                            <button onClick={() => setOpen((o) => ({ ...o, [b.id]: !o[b.id] }))} data-testid={`bitacora-toggle-${b.id}`} className="w-full px-5 py-3 flex items-center justify-between hover:bg-[#C8D4A0]/15 rounded-[10px]">
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="font-bold text-[#4B5828]">${b.costo_total_bitacora?.toFixed(2)}</span>
                                    ...
                                    {b.aplicaciones?.map((ap, i) => (
                                        <div key={i} className="border border-neutral-100 rounded-[10px]">
                                            <div className="px-4 py-2 bg-[#F5F5F0] flex items-center justify-between">
                                                ...
                                                <div className="text-sm font-semibold text-[#4B5828]">${ap.costo_total_aplicacion?.toFixed(2)}</div>
                                            </div>
                                            <table className="w-full text-xs">
                                                <thead className="bg-white text-neutral-500">
                                                    <tr><th className="text-left px-3 py-1.5">Producto</th><th className="text-right px-3 py-1.5">Dosis</th><th className="text-left px-3 py-1.5">Unidad</th><th className="text-right px-3 py-1.5">Cantidad</th><th className="text-right px-3 py-1.5">Costo</th></tr>
                                                </thead>
                                                <tbody>
                                                    {ap.productos.map((p, j) => (
                                                        <tr key={j} className="border-t border-neutral-100">
                                                            ...
                                                            <td className="px-3 py-1.5 text-right font-semibold text-[#4B5828]">${p.costo_linea?.toFixed(2)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
